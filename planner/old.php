...
function newAssignment() {
    ...
    echo "<div class='assignment' id='newAssignment' style='display:none'>\n";
    echo "<div class='color-id' style='background-color:#ffffff'>\n";
    echo "<form method='post' class='editElement'>\n";
    echo "<input type='text' size='30' name='txtName' value='Assignment'>\n";
    echo "<input type='datetime-local' name='dtDue' step='1' value='" . ($datetime->format("Y-m-d\TH:i:s")) . "'><br>";
    echo "<select name='newClass'>";
    foreach($data as $classID => $classOption) {
        echo "<option value='$classID'>{$classOption["name"]}</option>";
    }
    echo "</select>";
    echo "<input type='submit' value='✓'>";
    echo "<input type='hidden' name='hidAssignment' value='".count($assignment)."'>";
    echo "<input type='hidden' name='hidClass' value='{$class}'>";
    ...
    echo "</form>";
    echo "<div class='nonEditElement'>";
    echo "<h1>Assignment</h1>";
    echo "<h2>{$class} - {$data[$class]["name"]}</h2>";
    echo "</div>";
    echo "</div>";
    echo "<form method='post' id=\"addStep\" class=\"editElement\">";
    ...
    echo "<input type='hidden' name='hidFormType' value='markComplete'>";
    echo "<input type='hidden' name='hidAssignment' value='".count($assignment)."'>";
    echo "<input type='hidden' name='hidClass' value='{$class}'>";
    echo "<input type='submit' value='Done!' />";
    echo "</form>";

    echo "<p class='due'>Due: ".($datetime->format("Y/m/d h:i A"))."</p>";
    echo "</div>";

    array_push($data[$class]["assignments"], array("name"=>"Assignment Name", "due"=>$datetime->getTimestamp(), "steps"=>array(), "done"=>false));
}
...
<body class="<?=$edit ? "editing" : "notEditing"?>">
    <main>
        ...
        <div class='assignment' id='newAssignment' style='display:none'>
            <div class='color-id' style='background-color:<?=$data[array_key_first($data)]["color"];?>'>
                <form method='post' class="editElement">
                    ...
                    <select name="newClass" >
                        ...
                        foreach($data as $classID => $classOption) {
                            ...
                            if($classID==array_key_first($data))
                                echo "selected";
                    ...
                    <input type='hidden' name='hidAssignment' value='-1'>
                    <input type='hidden' name='hidClass' value='<?=array_key_first($data)?>'>
                ...
                <div class="nonEditElement">
                    ...
                    <h2><?=array_key_first($data)?> - <?=$data[array_key_first($data)]["name"]?></h2>
                </div>
            </div>
        ...
        <?php foreach($assignments as $assignment):?>
            ...
            <div class='assignment<?php if($date < new DateTime()) echo " late";?>'>
                <div class='color-id' style='background-color:<?= $data[$class]["color"]; ?>'>
                    <form method='post' class="editElement">
                        <input type='text' size='30' name='txtName' value='<?=$assignment["name"];?>'>
                        <input class='dueDate' type='datetime-local' name='dtDue' step='1' value='<?=$date->format("Y-m-d\TH:i:s")?>'>
                        <br>
                        <select name="newClass" >
                            <?php 
                            foreach($data as $classID => $classOption) {
            ...
            <!-- Echo a human readable date & time -->
            <p class='due'>Due: <?=$date->format("Y/m/d H:i")?></p>
        </div>
        <?php endforeach;?>
    </main>
    ...
    <script>
        ...
        function submitForm(formId) {
            $.post('index.php', $("#"+formId).serialize())
        }

        $("#addAssignment").submit((e) => {
            submitForm("addAssignment")

            newAssignment = $("#newAssignment").clone()
            $(newAssignment).appendTo("#addAssignment")

            date = new Date();
            $(newAssignment).children(".dueDate").value(date.toISOString());
            $(newAssignment).children(".due").append("Due: " + date.getFullYear()+"/"+date.getMonth()+"/"+date.getDate() + " " + date.getHours() + ":" + date.getMinutes())
